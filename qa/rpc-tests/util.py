...
def initialize_chain(test_dir):
    ...
    4 wallets.
    sidecoind and sidecoin-cli must be in search path.
    """

    if not os.path.isdir(os.path.join("cache", "node0")):
        # Create cache directories, run sidecoinds:
        sidecoinds = []
        for i in range(4):
            datadir = os.path.join("cache", "node"+str(i))
            os.makedirs(datadir)
            with open(os.path.join(datadir, "sidecoin.conf"), 'w') as f:
                f.write("regtest=1\n");
                f.write("rpcuser=rt\n");
                f.write("rpcpassword=rt\n");
                f.write("port="+str(START_P2P_PORT+i)+"\n");
                f.write("rpcport="+str(START_RPC_PORT+i)+"\n");
            args = [ "sidecoind", "-keypool=1", "-datadir="+datadir ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(START_P2P_PORT))
            sidecoinds.append(subprocess.Popen(args))
            subprocess.check_output([ "sidecoin-cli", "-datadir="+datadir,
...
def start_nodes(num_nodes, dir):
    # Start sidecoinds, and wait for RPC interface to be up and running:
    for i in range(num_nodes):
        datadir = os.path.join(dir, "node"+str(i))
        args = [ "sidecoind", "-datadir="+datadir ]
        sidecoind_processes.append(subprocess.Popen(args))
        subprocess.check_output([ "sidecoin-cli", "-datadir="+datadir,
                                  "-rpcwait", "getblockcount"])
...
def stop_nodes():
    for process in sidecoind_processes:
        process.kill()
